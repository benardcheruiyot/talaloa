...
3. **Create environment file:**
   ```bash
   echo "REACT_APP_API_URL=http://localhost:5005/api" > .env
   ```
...
- `BACKEND_PORT`: Unique backend port on the server (example: `5005`)
...
./scripts/setup_github_environments.sh \
   ...
   --prod-backend-port 5005 \
   --staging-backend-port 5006
```
...
.\scripts\setup_github_environments.ps1 `
   ...
   -ProdBackendPort 5005 `
   -StagingBackendPort 5006
```
